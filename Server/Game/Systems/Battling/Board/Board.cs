    ...
    public class Board {
        ...
        public Board(List<Entity> entities1, List<Entity> entities2) {
            ...
            foreach (var entity in entities1) {
                ...
                entity.GetComponent<StatsComponent>().Died += HandleUnitDied;
            }
        ...
        private void HandleUnitDied(Entity entity) {
            entity.GetComponent<StatsComponent>().Died -= HandleUnitDied;
            var location = entity.GetComponent<MovementComponent>().position.coords;
            RemoveUnit(location);
            UnitDied?.Invoke(location);

            var unitsByTeam = GetUnits().GroupBy(x => x.GetComponent<TeamComponent>().Team);
        ...
        public Entity FindEnemyInRange(HexCoords fromPosition, int attackRange, Guid team) {
            Entity entity = null;

            Queue<HexCoords> openSet = new();
            HashSet<HexCoords> closedSet = new();       
            foreach (var direction in HexCoords.directions) {
                openSet.Enqueue(fromPosition + direction);
            }

            while (openSet.Count > 0) {
                var coord = openSet.Dequeue();
                closedSet.Add(coord);

                if (HexCoords.Distance(fromPosition, coord) > attackRange)
                    continue;

                if (coord == fromPosition)
                    continue;

                var unit = GetUnit(coord);
                if (!(unit is null)) {
                    if (unit.GetComponent<TeamComponent>().Team != team) {
                        return unit;
                    }
                }

                foreach (var direction in HexCoords.directions) {
                    var newCoord = coord + direction;
                    if (!Contains(newCoord))
                        continue;

                    if (closedSet.Contains(newCoord))
                        continue;

                    openSet.Enqueue(newCoord);
                }
            }
        ...
        public override string ToString() {
            ...
            foreach (var hex in grid) {
                ...
                str += $"Axial: [{hex.coords.q}, {hex.coords.r}] Offset: [{offsetCoords.x}, {offsetCoords.y}] Unit: {(hex.unit is null ? "" : hex.unit.GetComponent<TeamComponent>().Team)}      ";
                ctr++;
                if (ctr % 8 == 0) {
                    str += "\n";
                }
